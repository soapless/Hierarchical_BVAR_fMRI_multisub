...
* hbvar_fmri()
 ...
 - Arguments:
	...
	pilot_prior:		a list of prior constants used for the pilot estimation. A list containing 4 elements: beta, phi, pi and omega. Default is list(phi=list(mean=rep(0, 2*(P*P)*L), var_cov=diag(rep(0.5^2, 2*(P*P)*L))), beta = list(mean=rep(0, 3*P), var_cov=diag(rep(50^2, 3*P))), pi=matrix(rep(1/(1+L),(1+L)*P*P),1+L), omega=c(nu=P+1, psi_omega = diag(P))). For beta and phi, it is a list with the 1st element being the vector of the mean and the 2nd element being the variance-covariance matrix for the vector phi (or beta). For pi, it is a (L+1) by (P^2) matrix, with each column being the probability vector for the multinomial distribution for each element of xi. For omega, it is a list with the 1st element being the DF argument and the 2nd element being the scale matrix argument of the Wishart distribution for Omega. 
	...
	mc.cores:		the number of CPUs to use for parallel computing (works only for Linux and Mac systems). Default is: min(length(data), detectCores())
